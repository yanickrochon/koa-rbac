...
function * findMatchingRuleWeight(ctxRbac, depth, roles, permissions, permissionMatches) {
  var i, ilen;
  var j, jlen;
  var k, klen;
  var role;
  ...
  for (i = 0, ilen = roles.length; i < ilen; ++i) {
    ...
    if (weight === false && role.inherited) {
      weight = yield findMatchingRuleWeight(ctxRbac, depth + 1, role.inherited, permissions, permissionMatches);
    }
  }
...
function * isAllowed(ctx, permissions, updateContext) {
  ...
  if (ctxRbac && ctxRbac.accessProvider) {
    ...
    //console.log("*** IS ALLOWED", permissions, ruleWeight, ctxRbac.allowedWeight);
...
function * isDenied(ctx, permissions) {
  ...
  if (ctxRbac && ctxRbac.accessProvider) {
    ...
    //console.log("*** IS DENIED", permissions, ruleWeight, ctxRbac.allowedWeight);
...
function deny(permissions, redirectUrl) {
  ...
  return function * denyPermissions(next) {
    ...
    //console.log("*** CHECK", permissions, deniedWeight, allowedWeight);
