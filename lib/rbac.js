...
function * findMatchingRuleWeight(depth, roles, provider, permissions) {
  ...
  for (i = 0, ilen = roles.length; i < ilen; ++i) {
    ...
    //console.log("Check inheritence", roles[i], role.inherits);
    if (weight === false && role.inherits) {
      inheritedWeight = yield findMatchingRuleWeight(depth + 1, role.inherits, provider, permissions);
...
function allow(permissions) {
  if (typeof permissions === 'string' && arguments.length === 1) {
    permissions = permissions.split(/\s+/);
  } else if (arguments.length > 1) {
    permissions = Array.prototype.slice.call(arguments);
